...
async function handleRequest(request: Request, env: Env) {
  ...
  if (
    ...
    authorizeParams.append("access_type", "offline");
    authorizeParams.append("prompt", "consent");
    ...
    await env.googleSheetsOAuthStore.put(`readKey:${writeKey}`, readKey, {
      expirationTtl: 60,
    });
  ...
  if (request.method === "GET" && requestUrl.pathname.startsWith("/redirect")) {
    ...
    const readKey = await env.googleSheetsOAuthStore.get(`readKey:${writeKey}`);
    ...
    await env.googleSheetsOAuthStore.put(
      `tokens:${readKey}`,
  ...
  if (request.method === "POST" && requestUrl.pathname.startsWith("/poll")) {
    ...
    const tokens = await env.googleSheetsOAuthStore.get(`tokens:${readKey}`);
    ...
    await env.googleSheetsOAuthStore.delete(`tokens:${readKey}`);
